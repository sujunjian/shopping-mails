        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(salelist,index) in spuSaleAttrList" :key="index">
                <dt class="title">{{ salelist.saleAttrName }}</dt>
                <dd changepirce="0" 
                :class="{active:saleattr.isChecked=='1'}" 
                v-for="(saleattr,index) in salelist.spuSaleAttrValueList" :key="saleattr.id"
                @click="changemethods(saleattr,salelist.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
<!-- 这里的路由跳转，在跳转之前先发送请求给cart路由通知服务器，服务器进行对应的数据存储 -->
...
import { mapGetters } from 'vuex';
  ...
  export default {
    ...
    mounted(){
      // 占位的地方是什么名字params就是什么名字
      this.$store.dispatch("getgoodsInfo",this.$route.params.id);
    },
    computed:{
      ...mapGetters(["categoryview","skuInfo","spuSaleAttrList","aluesSkuJson"])
    },
  methods: {
    changemethods(saleattr,arr){
// 有没有高量取决于ischeck是否为1 应该拿到数组的所有属性，然后使用排他思想
      arr.forEach(item => {
        item.isChecked='0';
      });
      // 使用排他思想，确定唯一那个值有高亮效果
    ...
    changeskuNum(event){
      //事件上有事件对象在event.target 表单元素值为value
      // *1可以检测元素是否为纯数字
      let value=event.target.value*1;
      value=parseInt(value);
      // 可以使用双向model上面的.number限制文本
      if(value>0 &&value){
        this.skuNum=value
      }else{
        // 不能出现小数点 只能是整数
    ...
    // 点击加入购物车 发送请求给服务器后端 
    async addshopcar(){
      // 把当前的产品信息带过去 id和num 判断加入成功或者失败不能使用存储数据
      // 返回的函数是promise
       // 既然返回的是成功或者失败的回调，那就可以使用try catch处理函数 
     try {
      let result=await this.$store.dispatch('addOrUpdateShopcar',{
        skuId:this.$route.params.id,
        skuNum:this.skuNum
      });
      // 添加成功，进行2路由的跳转 没有历史记录使用push没有历史记录
      // 在路由跳转的时候会带产品的信息过去，name price id 
      // 一些简单的数据【比较复杂，会话结束直接消失】
      // 不管是本地存储还是会话存储，存的都是字符串
            ...
            .chooseArea {
              ...
              dl {
                ...
                dd {
                  ...
                  &.active {
                    color: rgb(220, 70, 29);
                    border: 2px solid rgb(41, 187, 198);
                  }
                }
              }
            }
